...
export const SignatureCapture = (props: SignatureCaptureProps) => {
  ...
  const loadTopazWrapper = async () => {
    try {
      ...
      if (url) {
        ...
        script.src = url
        script.onload = () => {
          topazWrapper = (window as any).Topaz
          setStatus('Listo para conectar. Asegúrate de tener instalada la extensión Topaz SigPlusExtLite.')
        }
        script.onerror = () => {
          setStatus('Error: No se pudo cargar el wrapper de Topaz. Verifica que la extensión esté instalada.', true)
        }
        document.head.appendChild(script)
      } else {
        setStatus('Error: SigPlusExtLiteWrapperURL no encontrado. Instala la extensión Topaz.', true)
      }
    } catch (error) {
      setStatus(`Error al cargar Topaz: ${error}`, true)
    }
  }
  ...
  const connect = async () => {
    try {
      ...
      if (!topazWrapper) {
        throw new Error('Topaz object not available. Please install the Topaz SigPlusExtLite extension.')
      }
  ...
  return (
    <div class="signature-capture">
      ...
      <div class={`status ${status().includes('Error') ? 'error' : 'success'}`}>
        {status()}
      </div>
